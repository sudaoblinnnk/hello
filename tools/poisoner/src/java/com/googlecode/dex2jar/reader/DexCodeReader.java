...
import com.irdeto.secureaccess.android.dexreader.DexWriter;
import com.irdeto.secureaccess.android.dexreader.DexInfo;
import com.irdeto.secureaccess.android.dexreader.DexConfigure;
...
public class DexCodeReader implements DexOpcodes {
    ...
    private int offsetInstructionPerMethod = 0;
    
    private int lengthInstuctionPerMethod = 0;

    private byte NOP = 0x00;
    
    private DexWriter dexWriter;
    private DexConfigure dexConfig = null;
    
    private DexInfo dexInfo = null;
  
    /* default */
    public DexCodeReader(DexFileReader dex, DataIn in, boolean isStatic, Method method) {
        this.dex = dex;
        this.in = in;
        this.method = method;
        this.isStatic = isStatic;
    }
    public DexCodeReader(DexFileReader dex, DataIn in, boolean isStatic, Method method, DexWriter dexWriter, DexConfigure dexConfig) {
        this(dex, in, isStatic, method);
    ...
    public void accept(DexCodeVisitor dcv, int config) {
        ...
        int out_size = in.readUShortx();
        int tries_size = in.readUShortx();
        int debug_off = in.readUIntx();
        int instruction_size = in.readUIntx();

        dexInfo = dexWriter.getDexInfo();
        if(dexInfo == null){
        	System.out.println("Faild to get DexInfo!!");
        	return;
        }
        
        dexInfo.setRegisterSize(total_registers_size);
        dexInfo.setInArgsSize(in_register_size);
        dexInfo.setOutSize(out_size);
        dexInfo.setTriesSize(tries_size);
    ...
    private void acceptInsn(DataIn in, int instruction_size, DexOpcodeAdapter n) {
        ...
        int lengthInstuction = 0;
        int offsetInstruction = 0;
        
    	lengthInstuctionPerMethod = 0;
    	offsetInstructionPerMethod = in.getCurrentPosition();                	
    	String position = String.format("0x%x", offsetInstructionPerMethod);
    	DexConfigure.displayMessage("Code offset: "+position);
        ...
        for (int baseOffset = in.getCurrentPosition(), currentOffset = 0; currentOffset < instruction_size; currentOffset = (in
            ...
            offsetInstruction = in.getCurrentPosition();
            position = String.format("0x%x", offsetInstruction);
            int opcode = in.readUShortx();
            int uOpcodeH = opcode >> 8;
            {
                int uOpcodeL = opcode & 0xFF;
            ...
            lengthInstuction = 0;
            String code = String.format("0x%x", opcode);            
            DexConfigure.displayMessage("Instruction Opcode: "+code+" position: "+position);           
            dexWriter.getDexInfo().setOpcode(opcode);
            ...
            lengthInstuction += (in.getCurrentPosition() - offsetInstruction);
            DexConfigure.displayMessage("Bytecode length of the instruction: "+lengthInstuction);
            if(!dexConfig.isWholeMethodInstruction()){
            	dexWriter.write(NOP, lengthInstuction, offsetInstruction);
            }	
            lengthInstuctionPerMethod = (in.getCurrentPosition() - offsetInstructionPerMethod);
        ...
        DexConfigure.displayMessage("Total bytecode length for the method: "+lengthInstuctionPerMethod);
        if(dexConfig.isWholeMethodInstruction()){
        	dexWriter.write(NOP, lengthInstuctionPerMethod, offsetInstructionPerMethod);
        }      
    }
