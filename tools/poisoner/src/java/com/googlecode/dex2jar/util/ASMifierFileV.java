...
public class ASMifierFileV implements OdexFileVisitor {
	...
	public DexClassVisitor visit(int access_flags, String className,
		...
		final String n = String.format("C%04d_", i++)
				+ className.substring(1, className.length() - 1)
						.replace('/', '_').replace('$', '_');
		file.s("%s.accept(v);", n);
		return new ASMifierClassV(pkgName, n, access_flags, className,
				superClass, interfaceNames) {

			@Override
			public void visitEnd() {
				super.visitEnd();
				write(out, new File(dir, pkgName.replace('.', '/') + '/' + n
						+ ".java"));
			}
	...
	public void visitDepedence(String name, byte[] checksum) {
		file.s("((OdexFileVisitor)v).visitDepedence(%s,decodeHex(%s.toCharArray()));",
				Escape.v(name), Escape.v(new String(Hex.encodeHex(checksum))));
	}
